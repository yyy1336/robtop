#ifndef __GPU_VECTOR_CUH2
#define __GPU_VECTOR_CUH2
...
namespace gv2 {
	...
	public:
		...
		void lambda_test(void) {
			...
			gv2::map << <grid_size, block_size >> > (_size, kernel);
			cudaDeviceSynchronize();
			cuda_error_check;
		}
		...
		template<typename expr_t, typename T,
			...
			typename std::enable_if<std::is_scalar_v<T>, int>::type = 0
		>
			void set(const expr_t& exprfilter, const gv2::gVector<T>& repl_val) {
	...
	const gVector<Scalar>& gVector<Scalar>::operator/=(Scalar s) {
		...
		gv2::map << <grid_size, block_size >> > (_data, size(), [=] __device__(int tid) { return ptr[tid] / s; });
		cudaDeviceSynchronize();
		cuda_error_check;
		return *this;
	}
	...
	void gVector<Scalar>::invInPlace(void) {
		...
		size_t grid_size, block_size;
		make_kernel_param(&grid_size, &block_size, _size, 512);
		gv2::map << <grid_size, block_size >> > (ptr, size(), [=] __device__(int tid) { return 1 / ptr[tid]; });
		cudaDeviceSynchronize();
		cuda_error_check;
		return;
	}
	...
	const gVector<Scalar>& gVector<Scalar>::operator*=(Scalar s) {
		...
		size_t grid_size, block_size;
		make_kernel_param(&grid_size, &block_size, _size, 512);
		gv2::map<<<grid_size,block_size>>>(data(), size(), [=] __device__(int tid) { return ptr[tid] * s; });
		cudaDeviceSynchronize();
		cuda_error_check;
		return *this;
	}
	...
	void gVector<Scalar>::set(int* filter, Scalar val)
	{
		...
		gv2::map << <grid_size, block_size >> > (size(), [=] __device__(int tid) {
	...
	void gVector<Scalar>::maximize(Scalar s)
	{
		...
		size_t grid_size, block_size;
		make_kernel_param(&grid_size, &block_size, _size, 512);
		gv2::map<<<grid_size,block_size>>>(ptr, size(), [=]  __device__(int tid) {
	...
	void gVector<Scalar>::maximize(const gVector<Scalar>& v2)
	{
		...
		size_t grid_size, block_size;
		make_kernel_param(&grid_size, &block_size, _size, 512);
		gv2::map<<<grid_size,block_size>>>(v1data, size(), [=] __device__(int tid) {
	...
	void gVector<Scalar>::clamp(Scalar lower, Scalar upper)
	{
		...
		size_t grid_size, block_size;
		make_kernel_param(&grid_size, &block_size, _size, 512);
		gv2::map<<<grid_size,block_size>>>(ptr, size(), clamp_kernel);
		cudaDeviceSynchronize();
		cuda_error_check;
	}
	...
	void gVector<Scalar>::clamp(Scalar* lower, Scalar* upper)
	{
		...
		size_t grid_size, block_size;
		make_kernel_param(&grid_size, &block_size, _size, 512);
		gv2::map<<<grid_size,block_size>>>(ptr, size(), [=] __device__(int eid) {
			Scalar val = ptr[eid];
	...
	Scalar gVector<Scalar>::dot(const gVector<Scalar>& v2) const {
		Scalar* pbuf = gVector<Scalar>::get_dump_buf(sizeof(Scalar) * (size() / 300 + 1));
		return gv2::dot(data(), v2.data(), pbuf, size());
	}
	...
	Scalar gVector<Scalar>::min_positive(void) const {
		...
		gv2::map << <grid_size, block_size >> > (tmp.data(), size(), [=] __device__(int eid) {
			Scalar val = src[eid];
			if (val < 0) {
				val = 1e30;
			}
	...
	void gVector<Scalar>::Sqrt(void) {
		...
		make_kernel_param(&grid_size, &block_size, size(), 512);
		Scalar* src = _data;
		gv2::map << <grid_size, block_size >> > (_data, size(), [=] __device__(int tid) {
			return sqrt(src[tid]);
		});
		cudaDeviceSynchronize();
		cuda_error_check;
...
//constexpr bool ts = is_expression< var_t<gv2::gVector<double> > >::value;
