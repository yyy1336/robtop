...
void test_gVector(void)
{
	...
	v1.set(v1 * 2 >= 3 || v1 / 2 < 6, 0.333);
	//v1.set(v1 * 2 >= 3 ^ v1 / 2 < 6, 0.333);
	v1 = gv2::power(v1, 2) / v1 * 2;
	//constexpr bool truv = gv2::is_expression<decltype(v1 > 10)>::value;
	//typedef gv2::scalar_t<double, 0> Vd;
	//Vd v(10);
	//constexpr bool truv = gv2::is_expression<gv2::scalar_t<double>>::value;
	//v1.lambda_test();
	...
	gv2::gVector<double> v2(2);
	v2[0] = 1; v2[1] = 2;
	//gv2::gVector<double> v2arr = v2.dup(5, 2);
	gv2::gVector<double> v2arr = v2.concat(v2).concat(v2).concat(v2).concat(v1);
	for (int i = 0; i < v2arr.size(); i++) {
		std::cout << v2arr[i] << ",";
	}
	std::cout << std::endl;
#elif 0
	gv2::gVector<double> v1(100000);
	v1.set(1);
	v1.concat(v1 + 1).concat(v1 + 2).toMatlab("v1s");
	v1.pitchSumInPlace(1, 10000, 20000, true);
	v1.toMatlab("v1");
	gv2::gVector<double> v2(1000);
	...
	v1.set(0.5);
	gv2::gVector<double> v2(nbytepitch / sizeof(double));
	v2.set(2.);
	//v2.dup(100, 10000, wordpitch);
	gv2::gVector<double> v3 /*= v1 * v2.dup(100, 10000, wordpitch)*/;
	v3 = v1 * 2;
	v3.pitchSumInPlace(4, 10000, wordpitch);
	v3.toMatlab("v3");
	gv2::gVector<double> v4(100);
...
void matrix_AAt_rowmajor(const Scalar* pdata, int nwordvalid, int nwordpitch, int npitch, const WT* weight, Scalar* dst, int ndstwordpitch) {
	int nblockStride = gv2::round<BlockSize>(nwordvalid) / BlockSize;

	int nbatch = gv2::round<BatchSize>(npitch) / BatchSize;
...
std::tuple<gv2::gVector<double>, double, double> kktcheck(
	int nconstrain, int nvar,
	const gv2::gVector<double>& xmma, const gv2::gVector<double>& ymma, double z,
	const gv2::gVector<double>& lam, const gv2::gVector<double>& xsi, const gv2::gVector<double>& eta,
	const gv2::gVector<double>& mu, double zet, const gv2::gVector<double>& s,
	const gv2::gVector<double>& xmin, const gv2::gVector<double>& xmax,
	const gv2::gVector<double>& df0dx, const gv2::gVector<double>& gval, const gv2::gVector<double>& dgdx,
	int wordPitch,
	double a0, const gv2::gVector<double>& a, const gv2::gVector<double>& b, const gv2::gVector<double>& c, const gv2::gVector<double>& d
	)
{
	gv2::gVector<double> dgdxtmp(dgdx);
	dgdxtmp.weightedPitchSumInPlace(4, nvar, wordPitch, lam.data(), true);
	gv2::gVector<double> rex = df0dx + dgdxtmp.slice(0, nvar) - xsi + eta;
	...
	auto rezet = zet * z;
	auto res = lam * s;
	gv2::gVector<double> residu = rex.concat(rey, rez, relam, rexsi, reeta, remu, rezet, res);
...
void update_pqlambda_gvec(
	int nconstrain, int nvar,
	const gv2::gVector<double>& uxinv1, const gv2::gVector<double>& xlinv1,
	const gv2::gVector<double>& p0, const gv2::gVector<double>& q0,
	const gv2::gVector<double>& P, const gv2::gVector<double> & Q,
	const gv2::gVector<double>& lam,
	gv2::gVector<double>& plam, gv2::gVector<double>& qlam,
	gv2::gVector<double>& gmat, gv2::gVector<double>& gvec
) {
	cuda_error_check;
...
typedef std::tuple<
	gv2::gVector<double>, gv2::gVector<double>, double,
	gv2::gVector<double>, gv2::gVector<double>, gv2::gVector<double>,
	gv2::gVector<double>, double, gv2::gVector<double>
> mma_arg_pack;
...
mma_arg_pack subsolv_g(int nconstrain, int nvar, double epsimin,
	const gv2::gVector<double>& low, const gv2::gVector<double>& upp,
	const gv2::gVector<double>& alfa, const gv2::gVector<double>& beta,
	const gv2::gVector<double>& p0, const gv2::gVector<double>& q0,
	const gv2::gVector<double>& P, const gv2::gVector<double>& Q,
	int Pwordpitch, int Qwordpitch,
	double a0, const gv2::gVector<double>& a, const gv2::gVector<double>& b, const gv2::gVector<double>& c, const gv2::gVector<double>& d
) {
	typedef gv2::gVector<double> gVector;
...
gv2::gVector<double> mmasub_ker(int nconstrain, int nvar,
	int itn, gv2::gVector<double>& xvar_g,
	gv2::gVector<double>& xmin_g, gv2::gVector<double>& xmax_g,
	gv2::gVector<double>& xold1_g, gv2::gVector<double>& xold2_g,
	double f0val, gv2::gVector<double>& df0dx_g, gv2::gVector<double>& gval_g, gv2::gVector<double>& dgdx_g,
	gv2::gVector<double>& low_g, gv2::gVector<double>& upp_g,
	double a0, gv2::gVector<double>& a_g, gv2::gVector<double>& c_g, gv2::gVector<double>& d_g,
	double move, int dgdxvarpitch
) {
	using gVector = gv2::gVector<double>;
...
void mmasub_g(int nconstrain, int nvar,
	...
	gv2::gVector<double>::Init();

	typedef gv2::gVector<double> gVector;
	typedef gv2::gVectorMap<double> gVectorMap;
...
) 
{
	...
	cuda_error_check;
	gVector dgdx_g(gv2::gVectorMap<double>(gx, nvarpitch / sizeof(double) * nconstrain));
...
void test_extern(void) {
    homo::MMAOptimizer mma(2, 2097152, 1, 0, 1000, 1);
	mma.setBound(0.001, 1);
	printf("mma: a0=%.4f, a=%.4f ,c=%.4f ,d=%.4f ,nvar=%d, nconstrain=%d, gpitch=%d\n", \
	        mma.a0, mma.a, mma.c, mma.d, mma.n, mma.m, mma.context->gpitch); //虽然不知道gpitch是在哪算的，但既然已经算好了，那就不用管它
}
